...
libero_config_path = os.environ.get(
    "LIBERO_CONFIG_PATH", os.path.dirname(os.path.abspath(__file__))
)
...
if not os.path.exists(config_file):
    ...
    # Only prompt when running interactively; in headless jobs (e.g. SLURM)
    # fall back to the default package-relative paths.
    interactive = sys.stdin is not None and sys.stdin.isatty()
    answer = (
        input("Do you want to specify a custom path for the dataset folder? (Y/N): ").lower()
        if interactive
        else "n"
    )
    if answer == "y":
