...
# (shared with libero.libero: the package root, i.e. libero/libero)
_default_config_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
libero_config_path = os.environ.get("LIBERO_CONFIG_PATH", _default_config_path)
config_file = os.path.join(libero_config_path, "config.yaml")


def get_path_dict(root_location=_default_config_path):
...
def set_libero_path(custom_location=_default_config_path):
